...
static void sigchld_handler(int sig) {
  ...
  while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0) {
    for (int i = 0; i < njobmax; i++) {
      job_t *job = &jobs[i];
      for (int d = 0; d < job->nproc; d++) {
        proc_t *proc = &job->proc[d];
        if (proc->pid == pid) // state of this proc has changed in job[i]
        {
      ...
      int s = FINISHED;
      for (int d = 0; d < job->nproc;
           d++) { /// This for finds out current state of job[i]
        proc_t *proc = &job->proc[d];
        if (proc->state == RUNNING) {
          s = RUNNING;
          break;
        } else if (proc->state == FINISHED && s != STOPPED) {
          s = FINISHED;
        } else {
          s = STOPPED;
        }
      }
      if (s == STOPPED) { // Changing state of job[i]
        job->state = STOPPED;
      } else if (s == FINISHED) {
        job->state = FINISHED;
      } else {
...
int monitorjob(sigset_t *mask) {
  ...
  if (state == STOPPED) {
    int j = allocjob();
    movejob(FG, j);
    msg("[%d] suspended '%s'", j, jobs[j].command);
  }
