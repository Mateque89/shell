...
noreturn void external_command(char **argv) {
  ...
  if (!index(argv[0], '/') && path) {
    ...
    char *sliced_path = strdup(path);
    int semi_pos;
    while (strlen(sliced_path) > 0) {
      semi_pos = strcspn(sliced_path, ":");
      char *slice = strndup(sliced_path, semi_pos);
      strapp(&slice, "/");
      strapp(&slice, argv[0]);
      (void)execve(slice, argv, environ);
      if (strlen(sliced_path + semi_pos) > 0)
        sliced_path += semi_pos + 1;
      else
        sliced_path += semi_pos;
      free(slice);
    }
    free(sliced_path);
  } else {
